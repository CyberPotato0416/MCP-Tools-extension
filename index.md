...
| 工具名稱 | 狀態 | 功能簡介 |
| :--- | :--- | :--- |
| **SplitPipe** | WIP | **切管大師**。精準 5m 模組切分與背對背法蘭對接 (幾何演算法演進中)。 |
...
1.  **矩陣式分圖邏輯 (Grid-Matrix Focus)**：
    專攻超大型廠房（如 GigaFactory）的矩陣式分割，解決了大規模專案的裁切精度與批量放圖問題。
2.  **動態過濾與配置持久化 (Filtering & Persistence)**：
    `TemplateLevelManager` 展現了超越標準工具的 UI 交互，具備領域、類型篩選與跨工作階段的狀態記憶 (JSON Based)。
3.  **深度協定標註 (Protocol-Driven Tagging)**：
    `WizardPick` 內建的結構化編碼邏輯，是專為複雜機電系統 (LOD 350+) 交付標準量身打造。
4.  **構造級幾何切割 (Construction-First Splitting)**：
    `SplitPipe` 的法蘭間隙與模組化邏輯，是少見具備「實際安裝考量」的建模延伸套件。

---
**最新維護日期**：2026-04-24 (鋼彈聯名版更新)
